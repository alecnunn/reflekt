...
class TypeDescriptor {
 public:
  ...
  void add_property(const std::string &name, const std::string &type,
...
class TypeRegistry {
 ...
 public:
  ...
  [[nodiscard]] std::vector<PropertyDescriptor> get_all_properties(
 ...
 private:
  void collect_properties_recursive(
      const std::string &type_name,
      std::vector<PropertyDescriptor> &props) const {
...
class DynamicObject {
 ...
 public:
  explicit DynamicObject(std::string type_name)
      : type_name_(std::move(type_name)) {
    const auto all_props =
        TypeRegistry::instance().get_all_properties(type_name_);
  ...
  [[nodiscard]] PropertyValue get_property_variant(
  ...
  [[nodiscard]] bool is_type(const std::string &type_name) const {
    ...
    const auto target_props =
        TypeRegistry::instance().get_all_properties(type_name);

    for (const auto &target_prop : target_props) {
      bool found = false;
      for (const auto &our_prop : all_props) {
        if (our_prop.name == target_prop.name &&
            our_prop.type_name == target_prop.type_name) {
          found = true;
          break;
        }
      }
...
class ObjectFactory {
 public:
  static std::unique_ptr<DynamicObject> create(const std::string &type_name) {
    if (const auto type_desc = TypeRegistry::instance().get_type(type_name);
        !type_desc) {
      return nullptr;
    }
...
class PropertyFileParser {
 public:
  static std::unique_ptr<TypeDescriptor> parse_simple_format(
 ...
 private:
  ...
  static PropertyValue parse_default_value(const std::string &type,
...
void print_type_info(const std::string &type_name) {
  ...
  std::cout << "base: "
            << (type_desc->base_type_name.empty() ? "none"
  ...
  for (const auto &prop : all_props) {
    ...
    std::cout << "    default_value: "
              << property_value_to_string(prop.default_value) << "\n";
...
magical: bool = false
  ...
  if (auto parsed_type =
          PropertyFileParser::parse_simple_format(property_content)) {
  ...
  iterate_type_properties(
      "Player", [](const std::string &name, const std::string &type,
                   const PropertyValue &default_val, bool inherited) {
        std::cout << "  " << name << " (" << type
                  << ") = " << property_value_to_string(default_val);
  ...
  if (player) {
    iterate_object_properties(
        *player, [](const std::string &name, const PropertyValue &value) {
